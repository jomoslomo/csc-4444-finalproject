...
# Function to count the number of fingers up on the left hand
def count_fingers(landmarks):
    finger_tips = [8, 12, 16, 20]  # Landmarks for the finger tips (except thumb)
    finger_bases = [6, 10, 14, 18]  # Landmarks for the finger bases (except thumb)
    thumb_tip = 4  # Landmark for the thumb tip
...
ws.connect("ws://localhost:8081")  # Adjust the server URL accordingly
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            if handedness.classification[0].label == 'Left':
                # Count the number of fingers up on the left hand
                finger_count = count_fingers(hand_landmarks.landmark)
                
                # Send a message to the Node.js server only if finger count is not 0
                if finger_count != 0:
                    message = {"type": "select", "link": f"{finger_count}"}
                ...
                for landmark in hand_landmarks.landmark:
                    ...
                    for box_name, box in hit_boxes.items():
                        if box["x"] <= x <= x <= box["x"] + box["width"] and box["y"] <= y <= box["y"] + box["height"]:
                            # Send the hit box name as a message to the Node.js server
                            message = {"type": "rotate", "direction": box_name}
                            ws.send(json.dumps(message))
